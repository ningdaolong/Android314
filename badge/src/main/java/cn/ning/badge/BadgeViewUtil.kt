...
internal object BadgeViewUtil {
    ...
    fun createBitmapSafely(dragBadgeView: DragBadgeView, rect: Rect, retryCount: Int): Bitmap? {
        try {
            ...
            // 只裁剪徽章区域,不然会很卡
            return Bitmap.createBitmap(dragBadgeView.drawingCache, if (rect.left < 0) 0 else rect.left, if (rect.top < 0) 0 else rect.top, rect.width(), rect.height())
    ...
    fun getIntersectionPoints(pMiddle: PointF, radius: Float, lineK: Double?): Array<PointF> {
        val points = arrayOf(PointF(0f, 0f), PointF(0f, 0f))
